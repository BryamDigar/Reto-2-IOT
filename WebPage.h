  ...
  <script>
    ...
    function fetchData() {
      fetch('/data')
        ...
        .then(data => {
          ...
          document.getElementById('lcd').innerText = data.lcd;
    ...
    function updateHistoryTable() {
      ...
      historyData.slice(0, 10).forEach(record => {
        ...
        tempCell.innerText = record.temp.toFixed(1) + ' ℃';
        row.appendChild(tempCell);
        
        const humiCell = document.createElement('td');
        humiCell.innerText = record.humi.toFixed(1) + ' %';
        row.appendChild(humiCell);
        
        const coCell = document.createElement('td');
        coCell.innerText = record.co;
